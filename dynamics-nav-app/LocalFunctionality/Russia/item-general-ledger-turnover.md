...
Указанные ниже форма и отчеты позволяют создавать оборотную ведомость для товаров и материалов.  
  
-   Форма "Финансовый оборот для товара" - форма 12449  
  
-   Товар Оборот (Кол-во) - отчет 12469  
  
-   Товар Оборот Памятка - отчет 12489  
  
## <a name="item-general-ledger-turnover-form-12449"></a>Форма "Финансовый оборот для товара" (12449)  
 Форма "Финансовый оборот для товара" показывает оборот для товаров в количествах и затратах, сальдо на начало месяца, приход и расход, а также сальдо на конец месяца.  
  
 Форма "Финансовый оборот для товара" обычно создается ежемесячно, а также может быть создана на дату инвентаризации. Данные формы "Финансовый оборот для товара" сверяются с данными складского учета. Данные по затратам сверяются с чистыми изменениями и сальдо:  
  
-   счета 41, Товары  
  
-   счета 10, Материалы  
  
 Чтобы открыть форму "Финансовый оборот для товара"  
  
-   В разделе **Финансовый менеджмент** щелкните **Склад**, затем **Товары Оборотная ведомость по ГК**. На вкладке **Параметры** имеется возможность указать параметры форматирования, приведенные в следующей таблице.  
  
|||  
|-|-|  
|Поле|Описанием|  
|**«Фильтр по дате»**|Введите период, для которого анализируются данные. Можно использовать кнопки, расположенные в нижнем левом углу формы, или указать период вручную.|  
...
|**Фильтр глобального измерения 2**|Введите значение фильтра для анализа в фильтре "Глобал. Измерение 2".|  
 ...
 Форма содержит поля со сведениями, перечисленными в следующей таблице. Значения в этих полях изменить нельзя.  
...
|**Себестоимость на Начало**<br /><br /> **Затраты на Конец**<br /><br /> **Дебет Затраты**<br /><br /> **Кредит Затраты**|В этих полях содержатся сведения о себестоимости товара — себестоимость на начало и на конец периода, а также приход и расход в финансовых суммах. Значения в этих полях вычисляются на основе сумм себестоимости, учтенных в операции ГК.|  
  
 Чтобы открыть выбранную карточку товара:  
  
-   Нажмите кнопку **Товар**, а затем выберите **карточка** (сочетание клавиш Shift+F5).  
  
 Чтобы напечатать отчеты "Товар Оборот (Кол-во)" или "Товар Оборот Памятка":  
  
-   Щелкните **Печать**, а затем **Оборот Листок** или **Оборот Памятка**.  
  
## <a name="item-turnover-qty-report-12469"></a>Отчет "Товар Оборот (Кол-во)" (12469)  
 Для анализа чистых изменений и сальдо товаров и материалов можно напечатать отчет "Товар Оборот (Кол-во)" и экспортировать его в Microsoft Office Excel. В этом отчете печатаются следующие данные из формы "Финансовый оборот для товара":  
...
1.  В **Финансах** щелкните **Склад**, затем **Финансовый оборот для товара**.  
  
2.  Щелкните **Печать**, а затем **Оборот Листок**.  
  
 Отчет "Оборот Листок" печатается ежемесячно и на дату инвентаризации, но может быть напечатан для любого данного периода.  
  
 На вкладке **Товар** формы запроса содержатся поля, перечисленные в следующей таблице.  
...
|**Склад Фильтр**|Введите код склада, для которого требуется фильтровать информацию, чтобы печатать форму "Финансовый оборот для товара" для одного или нескольких складов.<br /><br /> Можно ввести до 10 символов, включая цифры и буквы. Значение поля вводится автоматически в поле **Склад Фильтр**|  
...
2.  Щелкните **Печать**, а затем **Оборот Памятка**.  
...
|**«Фильтр по дате»**|Введите период, для которого печатается отчет. Данное поле является обязательным.|  
 ...
 На вкладке **Параметры** можно указать параметры форматирования, перечисленные в следующей таблице.  
